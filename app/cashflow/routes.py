...
def index():
    ...
    first_day_of_month, last_day_of_month, _,_= get_month_value(selected_month)
    if not Cashflow.query.filter(Cashflow.date == first_day_of_month).first():
        init_current_month()
    check_last_month_balance(selected_month)
...
def check_last_month_balance(date):
    ...
    last_record = Cashflow.query.filter(Cashflow.date>= firstday_of_previous_month,Cashflow.date <= lastday_of_previous_month).order_by(Cashflow.date.desc()).all()

    if last_record and cashflow:
        if not equal(float(last_record[0].balance),float(cashflow.balance)):
            print("last balance record: ", last_record[0].balance)
            old_balance = cashflow.balance
            cashflow.debit = float(last_record[0].balance)
            db.session.commit()
            print(cashflow.debit)
            cashflow.balance = float(cashflow.debit) - float(cashflow.credit)
            db.session.commit()
            ...
            for record in affected_records:
                if record.id != cashflow.id:
                    balance_change = cashflow.balance - old_balance
