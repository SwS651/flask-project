...
def search_stock():
    search_term = request.args.get('search_words','')

    print('Print',search_term)
    if search_term:
        results = Stock.query.filter(
            # Stock.bar_code.like("%"+search_term+"%") |
            Stock.StockName.ilike("%"+search_term+"%") 
            # Stock.category.like("%"+search_term+"%")
        ).all()
        # results = db.session.execute(db.select(Stock).filter(
        #     # Stock.bar_code.like("%"+search_term+"%") |
        #     Stock.StockName.like("%"+search_term+"%") 
        #     # Stock.category.like("%"+search_term+"%")
        # ))
    ...
    column_names = Stock.metadata.tables['stock'].columns.keys()
    return render_template('stock/index.html',stocks = results,column_names = column_names, search_term = search_term )
